...
public class ClassDiagramPanel extends JPanel {
    ...
    public ClassDiagramPanel(String name, List<ClassInfo> classes) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int segmentLength = sideLength * 4; 
        double step = (double) segmentLength / numClasses; 

        for (int i = 0; i < numClasses; i++) {
            double position = i * step;
            Point location = calculateSquarePosition(centerX, centerY, sideLength, position);
    ...
    private Point calculateSquarePosition(int centerX, int centerY, int sideLength, double position) {
        int halfSide = sideLength / 2;

        if (position <= sideLength) { 
            return new Point(centerX - halfSide + (int) position, centerY - halfSide);
        } else if (position <= 2 * sideLength) { 
            return new Point(centerX + halfSide, centerY - halfSide + (int) (position - sideLength));
        } else if (position <= 3 * sideLength) { 
            return new Point(centerX + halfSide - (int) (position - 2 * sideLength), centerY + halfSide);
        } else { 
            return new Point(centerX - halfSide, centerY + halfSide - (int) (position - 3 * sideLength));
        }
    }
    ...
    private int drawMethods(Graphics2D g2, List<MethodInfo> methods, int x, int y, int width) {
        int maxMethods = 10; // Maximum methods to display
        int methodHeight = Math.min(methods.size(), maxMethods) * 15;
        g2.setFont(new Font("SansSerif", Font.PLAIN, 10)); 
        int currentY = y + 12;

        for (int i = 0; i < Math.min(methods.size(), maxMethods); i++) {
            g2.drawString(methods.get(i).getRepresentation(), x + 5, currentY);
            currentY += 15;
        }

        
        if (methods.size() > maxMethods) {
            g2.drawString("... + " + (methods.size() - maxMethods), x + 5, currentY);
            currentY += 15; 
            methodHeight += 15; 
        }
